...
def spiderPic(html, keyword):
    ...
    for addr in re.findall('"objURL":"(.*?)"', html, re.S):
        ...
        try:
            pics = requests.get(addr, timeout=10) # 请求URL地址（最大时间(超时时间)10s）
        except requests.exceptions.ConnectionError:
            print('您当前的URL地址请求错误！')
            continue
        fq = open('E:\\pic\\' + (keyword + '_' +  str(random.randrange(0,1000,4)) + '.jpg'), 'wb')
...
if __name__ == '__main__':
    ...
    #results = requests.get('https://image.baidu.com/search/flip?tn=baiduimage&ie=utf-8&word=' +  word)
    results = requests.get('https://image.baidu.com/search/flip?tn=baiduimage&ie=utf-8&word=' + word)
# 调用函数
spiderPic(results.text, word)